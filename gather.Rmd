...
# adding year variable and adjusting for inflation
...
#creating function to pull and tidy mls payroll data from PDF tables compiled by
#MLSPA which list every player's salary year-by-year
...
s2016 <- pull_mls_payroll(2016, "http://s3.amazonaws.com/mlspa/September-15-2016-Salary-Information-Alphabetical.pdf?mtime=20180416202344")
...
s2017 <- pull_mls_payroll(2017, "http://s3.amazonaws.com/mlspa/September-15-2017-Salary-Information-Alphabetical.pdf?mtime=20180416202256")
...
# function didn't work smoothly with 2018, so did it manually
...
# filtering to remove salary groups not belonging to teams (e.g. "Pool",
# "Retired", etc.) - found break point by arranging to find lowest total that
# actually reflected a proper team payroll and lopping off everything below that
# cutoff

mls_payroll <- mls_salaries %>% 
  group_by(year, club) %>% 
  summarize(payroll = sum(compensation, na.rm = TRUE)) %>% 
  filter(payroll > 1666000,
...
save(nhl_adjusted, file = "data-files/nhl&mls.Rdata")
