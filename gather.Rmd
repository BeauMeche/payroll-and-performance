...
mlb_adjusted %>% 
  summarize(cor(payroll_adjusted, rs_win_pct)) # cor = .262
...
nba_adjusted %>% 
  summarize(cor(payroll_adjusted, rs_win_pct, us = "complete.obs")) # cor: .113
...
nba_adjusted %>% 
  ggplot(aes(season, payroll_adjusted / 1000, color = performance)) +
  geom_point(na.rm = TRUE)

mlb_adjusted %>% 
  ggplot(aes(year, payroll, color = ws_win)) +
  geom_point()

mlb_adjusted %>% 
  select(year, team_id, payroll, payroll_adjusted)
